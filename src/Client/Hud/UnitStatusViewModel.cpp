...
UnitStatusViewModel UnitStatusViewModelBuilder::build(const simulation::Entity& entity,
    ...
    model.displayNameKey = definition.uiNameKey;
    model.secondaryNameKey = definition.secondaryUiNameKey;
    ...
    const gamedata::VeterancyLevelDefinition* next = veterancy.nextLevel(entity.veterancyProfile,
        entity.experience);
    model.veterancyNameKey = current == nullptr ? entity.veterancyLevel : current->uiNameKey;
    ...
    model.armor.uiNameKey = definition.armor.uiNameKey.empty() ? definition.armorType : definition.armor.uiNameKey;
    ...
    for (const gamedata::WeaponDefinition& weapon : definition.weapons) {
        model.weapons.push_back({weapon.id, weapon.uiNameKey, weapon.icon, weapon.damage, weapon.range,
    ...
    if (!model.armor.uiNameKey.empty()) {
        model.tags.push_back(model.armor.uiNameKey);
    }
    return model;
}

std::wstring UnitStatusViewModelBuilder::tooltip(const ArmorCardViewModel& card) {
    return ra2yr::utf8ToWide(card.uiNameKey) + L"\n护甲：" + std::to_wstring(card.value) +
